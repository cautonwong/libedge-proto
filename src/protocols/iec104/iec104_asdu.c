...
edge_error_t edge_iec104_build_type1(edge_vector_t *v, uint32_t ioa, bool val, uint8_t qds) {
    EDGE_ASSERT_OK(edge_vector_put_u8(v, (uint8_t)(ioa & 0xFF)));
    EDGE_ASSERT_OK(edge_vector_put_u8(v, (uint8_t)((ioa >> 8) & 0xFF)));
    EDGE_ASSERT_OK(edge_vector_put_u8(v, (uint8_t)((ioa >> 16) & 0xFF)));
...
edge_error_t edge_iec104_build_type30(edge_vector_t *v, uint32_t ioa, bool val, const uint8_t *cp56) {
    EDGE_ASSERT_OK(edge_iec104_build_type1(v, ioa, val, 0));
