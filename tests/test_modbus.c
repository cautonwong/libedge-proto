...
static void test_build_read_holding_registers_req_rtu(void **state) {
    ...
    edge_error_t err = edge_modbus_build_read_holding_registers_req(&ctx, address, quantity, iov, &iov_count);
    assert_int_equal(err, EDGE_OK);
    assert_int_equal(iov_count, 1);
    ...
    assert_int_equal(get_iovec_total_len(iov, iov_count), sizeof(expected_frame));

    uint8_t actual_frame[sizeof(expected_frame)];
    combine_iovec(iov, iov_count, actual_frame);
...
static void test_build_write_single_register_req_tcp(void **state) {
    ...
    edge_error_t err = edge_modbus_build_write_single_register_req(&ctx, address, value, iov, &iov_count);
    assert_int_equal(err, EDGE_OK);
    assert_int_equal(iov_count, 1);
...
static void test_build_read_coils_req_tcp(void **state) {
    ...
    edge_error_t err = edge_modbus_build_read_coils_req(&ctx, 20, 10, iov, &iov_count);
    assert_int_equal(err, EDGE_OK);
    assert_int_equal(iov_count, 1);

    uint8_t expected[] = {0x00, 0x65, 0x00, 0x00, 0x00, 0x06, 0x05, 0x01, 0x00, 0x14, 0x00, 0x0A};
    uint8_t actual[sizeof(expected)];
    combine_iovec(iov, iov_count, actual);
...
static void test_build_read_discrete_inputs_req_tcp(void **state) {
    ...
    edge_error_t err = edge_modbus_build_read_discrete_inputs_req(&ctx, 30, 12, iov, &iov_count);
    assert_int_equal(err, EDGE_OK);
    assert_int_equal(iov_count, 1);
...
static void test_build_write_multiple_coils_req_tcp(void **state) {
    ...
    edge_error_t err = edge_modbus_build_write_multiple_coils_req(&ctx, 19, 10, data, iov, &iov_count);
    assert_int_equal(err, EDGE_OK);
    assert_int_equal(iov_count, 1);
...
static void test_build_write_multiple_registers_req_tcp(void **state) {
    ...
    edge_error_t err = edge_modbus_build_write_multiple_registers_req(&ctx, 100, 2, data, iov, &iov_count);
    assert_int_equal(err, EDGE_OK);
    assert_int_equal(iov_count, 1);

    // TID(401) Proto(0) Len(11) UID(1) FC(16) Addr(100) Qty(2) Bytes(4) Data(0A0B, 0C0D)
...
// Test case for parsing a "Read Coils" response
static void test_parse_read_coils_response(void **state) {
    (void) state;
    callback_was_called = false;
    edge_modbus_context_t ctx;
    edge_modbus_init_context(&ctx, 1, true);
    edge_modbus_callbacks_t cbs = {.on_read_coils_response = mock_on_read_coils_response};

    // Fake Response: TID(1) Proto(0) Len(5) UID(1) FC(1) ByteCount(2) Data(CD, 01)
    uint8_t frame[] = {0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x01, 0x01, 0x02, 0xCD, 0x01};
    edge_error_t err = edge_modbus_parse_frame(&ctx, frame, sizeof(frame), &cbs);
    assert_int_equal(err, EDGE_OK);
    assert_true(callback_was_called);
    assert_int_equal(captured_byte_count, 2);
    uint8_t expected_data[] = {0xCD, 0x01};
    assert_memory_equal(captured_data, expected_data, 2);
}

// Mock callback for write single coil response
static void mock_on_write_single_coil_response(edge_modbus_context_t *ctx, uint16_t address, uint16_t value) {
    (void) ctx;
    callback_was_called = true;
    captured_write_address = address;
    captured_write_value = value;
...
// Test case for parsing a "Write Single Coil" response
static void test_parse_write_single_coil_response(void **state) {
    (void) state;
    callback_was_called = false;
    edge_modbus_context_t ctx;
    edge_modbus_init_context(&ctx, 1, true);
    edge_modbus_callbacks_t cbs = {.on_write_single_coil_response = mock_on_write_single_coil_response};

    // Fake Response: TID(1) Proto(0) Len(6) UID(1) FC(5) Addr(20) Value(FF00)
    uint8_t frame[] = {0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x01, 0x05, 0x00, 0x14, 0xFF, 0x00};
    edge_error_t err = edge_modbus_parse_frame(&ctx, frame, sizeof(frame), &cbs);
    assert_int_equal(err, EDGE_OK);
    assert_true(callback_was_called);
    assert_int_equal(captured_write_address, 20);
    assert_int_equal(captured_write_value, 0xFF00); // 0xFF00 for ON
}

// Mock callback for write multiple coils response
static void mock_on_write_multiple_coils_response(edge_modbus_context_t *ctx, uint16_t address, uint16_t quantity) {
    (void) ctx;
    callback_was_called = true;
    captured_write_address = address;
    captured_write_quantity = quantity;
...
// Test case for parsing a "Write Multiple Coils" response
static void test_parse_write_multiple_coils_response(void **state) {
    (void) state;
    callback_was_called = false;
    edge_modbus_context_t ctx;
    edge_modbus_init_context(&ctx, 1, true);
    edge_modbus_callbacks_t cbs = {.on_write_multiple_coils_response = mock_on_write_multiple_coils_response};

    // Fake Response: TID(1) Proto(0) Len(6) UID(1) FC(15) Addr(19) Qty(10)
    uint8_t frame[] = {0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x01, 0x0F, 0x00, 0x13, 0x00, 0x0A};
    edge_error_t err = edge_modbus_parse_frame(&ctx, frame, sizeof(frame), &cbs);
    assert_int_equal(err, EDGE_OK);
    assert_true(callback_was_called);
    assert_int_equal(captured_write_address, 19);
    assert_int_equal(captured_write_quantity, 10);
}
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_parse_read_coils_response),
        cmocka_unit_test(test_parse_write_single_coil_response),
        cmocka_unit_test(test_parse_write_multiple_coils_response),
    };
